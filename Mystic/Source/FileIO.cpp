...
_static void Importer::ImportScene(std::string _file, std::vector<Mesh> &_mesh, std::vector<Material> &_materials)
{
...
_static void Importer::LoadMaterials(std::string _file, const aiScene * RawScene, std::vector<Material> &_materials)
{
  ...
  //_
		...
		std::string Name = fs::path(Path.C_Str()).filename().string();
 		std::string RealPath = Name;//  std::string((char*)Name.c_str());
		RealPath = _file.substr(0, _file.find_last_of("/\\")) + Name; // Find the Directory of the File we are parsing
...
_static void Importer::LoadMeshes(std::string _file, const aiScene *_scene, std::vector<Mesh> & _mesh)
{
...
_static void ConvertFile(std::string _fileIN, std::string _fileOUT)
{
